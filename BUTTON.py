...
from sql_lite_helper import insert_data_to_table
...
class ConnectionDock(QDockWidget):
    ...
    def push_button_click(self):
        ...
        if CURRENT_STATE == 'OFF':
            self.mc.publish_to("pr/home/button_123_BS/sts", 'ON')
            CURRENT_STATE = 'ON'
        else:
            self.mc.publish_to("pr/home/button_123_BS/sts", 'OFF')
            CURRENT_STATE = 'OFF'

    def send_message_to_relay(self, m_decode):
        splitted = m_decode.split(' ')
        temp_threshold = os.environ.get("TEMP_THRESHOLD", 22.5)
        if float(splitted[1]) > temp_threshold and CURRENT_STATE == 'OFF':
            print("in splitted")
            self.push_button_click()
        elif CURRENT_STATE == 'ON' and float(splitted[1]) <= temp_threshold:
            self.push_button_click()
